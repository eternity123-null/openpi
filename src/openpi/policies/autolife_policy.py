...
class AutolifeInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # state:20 + eef_pos:6 = 26
        state = transforms.pad_to_dim( np.concatenate([
            data["state"], data["eef_pos"][:3],data["eef_pos"][7:10]
        ]).astype(np.float32), self.action_dim)
        ...
        base_image = _parse_image(data["head_left_image"])
        left_wrist_image = _parse_image(data["hand_left_image"])
        right_wrist_image = _parse_image(data["hand_right_image"])
