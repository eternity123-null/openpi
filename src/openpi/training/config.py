...
import openpi.policies.fetch_policy as fetch_policy
...
class DataConfigFactory(abc.ABC):
    ...
    def create_base_config(self, assets_dirs: pathlib.Path) -> DataConfig:
        ...
        asset_id = self.assets.asset_id or "+".join(sorted(repo_ids)) if repo_ids else None
    ...
    def _load_norm_stats(self, assets_dir: epath.Path, asset_id: str | None) -> dict[str, _transforms.NormStats] | None:
        if asset_id is None:
            return None
        try:
            combined_dir = str(assets_dir / asset_id)
            norm_stats = _normalize.load(_download.maybe_download(combined_dir))
...
class FakeDataConfig(DataConfigFactory):
    repo_ids: list[str] = dataclasses.field(default_factory=lambda: ["fake"])

    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        return DataConfig(repo_ids=self.repo_ids)
...
@dataclasses.dataclass(frozen=True)
class LeRobotFetchDataConfig(DataConfigFactory):
    """
    This config is used to configure transforms that are applied at various parts of the data pipeline.
    For your own dataset, you can copy this class and modify the transforms to match your dataset based on the
    comments below.
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # replace the transforms below with your own.
        data_transforms = _transforms.Group(
            inputs=[fetch_policy.FetchInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[fetch_policy.FetchOutputs()],
        )
...
_CONFIGS = [
    TrainConfig(
        ...
        data=LeRobotFetchDataConfig(
            repo_ids=["place/place_bowl","place/place_cracker_box"],
            base_config=DataConfig(
                local_files_only=True,  # Set to True for local-only datasets.
                prompt_from_task=True,
            ),
            assets=AssetsConfig(
                assets_dir="/home/hairuo/VLA/assets/pi0_fetch_lora",
                asset_id="place_bowl+place_cracker_box",
            ),
            use_delta_joint_actions=True
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("/data/model/openpi-assets/checkpoints/pi0_base/params"),
